...
/*********************** ADDED BY treby BEGIN ******************************/

uint32_t PN532::readMfulPages(uint8_t cardnumber /*1 or 2*/,uint8_t startPageAddress /*0 to 15*/, uint8_t * pages) {
    pn532_packetbuffer[0] = PN532_INDATAEXCHANGE;
    pn532_packetbuffer[1] = cardnumber;  // either card 1 or 2 (tested for card 1)
    pn532_packetbuffer[2] = PN532_MIFARE_READ;
    pn532_packetbuffer[3] = startPageAddress; //This address can be 0-15 for MIFARE Ultralight
    ...
    for(uint8_t i=8;i<18+6;i++)
    {
        pages[i-8] = pn532_packetbuffer[i];
#ifdef PN532DEBUG
        Serial.print(pn532_packetbuffer[i], HEX); Serial.print(" ");
#endif
    }
...
uint32_t PN532::writeMfulPage(uint8_t cardnumber /*1 or 2*/,uint8_t pageAddress /*0 to 15*/, uint8_t * page) {
    pn532_packetbuffer[0] = PN532_INDATAEXCHANGE;
    pn532_packetbuffer[1] = cardnumber;  // either card 1 or 2 (tested for card 1)
    pn532_packetbuffer[2] = PN532_MIFARE_NEW_WRITE;
    pn532_packetbuffer[3] = pageAddress;

    for(uint8_t byte=0; byte <4; byte++)
    {
        pn532_packetbuffer[4+byte] = page[byte];
    }
